...
· 雪崩：大量 key 同时过期或宕机 → 随机过期 + 集群

---

📚 安全 | JWT 鉴权原理：你的令牌到底靠不靠谱？

想象你去公司上班，门禁怎么确认身份？不是每次都让你填工号+密码——给你一张工卡，刷卡就行。JWT 就是互联网世界的「工卡」。
...
· Payload：{"userId":1001,"role":"ENGINEER","exp":1716739200} — 你的身份信息
· Signature：HMAC-SHA256(header+"."+payload, secret) — 防伪芯片

核心逻辑：服务器不存 Session，全靠签名验证。收到请求 → 重新算签名 → 和 Token 第三段比对 → 一致就放行。

```bash
# 解码 JWT payload（仅查看，不验签）
...
为什么 RTD 半导体系统爱用？RTD 涉及十几个微服务——派工引擎、设备跟踪、Recipe 管理、FOUP 追踪。服务间互相调用频繁，如果用 Session，每次都得去 Redis 查，多一次网络 IO。JWT 无状态，服务自己验签就行，延迟低、扩展好。

⚠️ 两个坑：
1. Payload 只是 Base64 编码，不是加密！任何人都能解码，别放密码进去
2. 一定要设 exp 过期时间，否则 Token 泄露就是永久通行证

💡 记住一句话：JWT 的核心是「签名验证替代 Session 查询」——工卡内置防伪芯片，门禁不用每次都打电话到 HR 确认。

❓ 今日一题
Q: JWT 的签名（Signature）用来解决什么问题？
A) 加密传输内容，防止中间人窃听
B) 验证 Token 完整性，防止 Payload 被篡改
C) 压缩数据传输，减少网络开销
D) 实现 Token 自动续期
...
### MySQL 索引原理与 EXPLAIN 执行计划

上期答案: B — JWT 的 Signature（签名）部分负责验证数据完整性。Header + Payload 经 Base64 编码后用 Secret 签名，任何篡改都会导致签名不匹配，JWT 直接判定无效。

📚 数据库 | MySQL 索引原理与 EXPLAIN 执行计划

你有没有遇到过这种情况：一个 SQL 查询在测试环境跑得飞快，上了生产就慢得像乌龟？答案通常藏在索引和执行计划里。

打个比方：数据库表就像一本 1000 页的书。没有索引，你要找「第三章第 5 节」只能从头翻到尾——这叫全表扫描。有了索引，就像书前的目录，直接翻到对应页码。

MySQL 默认用 B+Tree 索引。想象一棵倒立的树：根节点在顶上，叶子在最下面。每个节点存的是「键值 + 指针」，叶子节点之间还手拉手（双向链表），所以范围查询也很快。比如 WHERE id BETWEEN 100 AND 200，定位到 100 后顺着链表一扫就全拿到了。

在半导体 RTD 场景里，典型查询是：「查询设备 EQ-01 在 2026-05-26 的所有 trace 数据」。trace 表可能有上亿行，怎么建索引？

-- ❌ 错误的建法
CREATE INDEX idx_time ON trace_data(collect_time);

-- ✅ 正确的建法：设备ID在前，时间在后
CREATE INDEX idx_eq_time ON trace_data(equipment_id, collect_time);

为什么？因为查询里 equipment_id 是等值过滤，collect_time 是范围过滤。联合索引把等值放前面，B+Tree 能精准定位到 EQ-01 的叶子节点，再按时间范围扫描。反过来时间放前面，跨设备数据混在一起，要多扫很多页。

怎么看索引用没用上？EXPLAIN 是你的眼睛：

EXPLAIN SELECT * FROM trace_data
WHERE equipment_id = 'EQ-01'
  AND collect_time >= '2026-05-26 00:00:00';

三个关键字段：type（访问方式，ALL=全表扫→坏信号）、key（实际使用的索引，NULL=没用上）、Extra（Using filesort/Using temporary=坏信号）。看到 type=ALL 就要警觉——在 RTD 环境里一个 ALL 扫描 5000 万行 trace，可能让 MES 系统超时。

排障思路：EXPLAIN → type=ALL → 建索引 → 再 EXPLAIN 验证。

💡 记住一句话：索引是空间换时间的艺术——建对了查询飞快，建错了反而拖慢写入。

❓ 今日一题
Q: 联合索引 (A, B, C) 对以下哪个查询生效？
A) WHERE B = 1 AND C = 2
B) WHERE C = 2
C) WHERE A = 1 AND B = 2
D) WHERE B = 1
...
## 17:02 | Linux System Administration

### Linux File Descriptors: Too Many Open Files Explained

Last Answer: C - For composite index (A, B, C), WHERE A=1 AND B=2 hits the index due to leftmost prefix rule; WHERE B=1 AND C=2 skips A so index fails.

=== Linux: File Descriptors ===

Imagine you're a waiter who can carry at most 6 plates. Suddenly 8 tables order at once - you can't handle it. "Too many open files" means you've run out of "plates" (file descriptors).

In Linux, everything is a file: network sockets, pipes, regular files, even /proc entries. In RTD, an EAP service may maintain dozens of TCP connections to equipment, several DB connection pools, plus log files - all file descriptors.

Quick diagnosis:
  ls /proc/$PID/fd | wc -l        # how many fds opened
  cat /proc/sys/fs/file-max        # system-wide limit
  cat /proc/$PID/limits | grep 'open files'  # per-process limit

For RTD: 200 equipment connections + 20 DB pool + 5 logs = 225 fds easily. Default ulimit -n is usually 1024, seems enough, but double connections or fd leaks (forgot close) triggers the error.

Tuning:
  ulimit -n 65536                       # temporary
  /etc/security/limits.conf            # permanent
  systemd: LimitNOFILE=65536 in [Service]

Golden rule: Linux "everything is a file" is not philosophy - every TCP connection and log handle consumes your fd quota.

Q: Java app via systemd with LimitNOFILE=65536, but Runtime.exec('ulimit -n') still shows 1024. Most likely reason?
A) systemd doesn't support LimitNOFILE
B) JVM resets ulimit on startup
C) ulimit -n is a shell builtin; Runtime.exec() spawns a child shell with its own limits
D) /etc/security/limits.conf overrides systemd
