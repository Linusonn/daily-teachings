...
上期答案: C — docker logs 默认读取容器的 stdout 和 stderr。它不是去翻容器内的 /var/log 文件，也不是递归扫描——只收集进程的标准输出和标准错误流。这就是为什么 Java 应用如果只用 logback 写文件而不配 ConsoleAppender，docker logs 看到的就是一片空白。

---

📚 [Linux] /proc 伪文件系统：RTD 排障的「X光机」

你负责的 RTD 派工服务突然假死了——进程还活着但不干活。没有日志报错，top 看着也正常。这时候怎么办？

Linux 给你准备了一台「X光机」：/proc 伪文件系统。它不是真实的磁盘文件，而是内核在内存里实时「投影」出的进程和系统状态。相当于你可以直接翻开内核的笔记本，看它记录的每一个细节。

② 进进程内部「体检」—— /proc/PID/

假设你的 RTD 服务 PID 是 12345：

```bash
# 1. 看基本健康指标：内存占用、线程数、运行状态
cat /proc/12345/status | grep -E "^(VmRSS|VmSize|Threads|State)"
# VmRSS: 1.2 GB   → 物理内存占用
# Threads: 47      → 开了多少个线程（RTD 线程池是不是炸了？）
# State: S(sleeping) → 正常，D 的话在等 IO 卡住了

# 2. 看打开了多少文件描述符——每个 socket 连接都是一个 fd
ls /proc/12345/fd/ | wc -l
# 如果结果是 1024（默认上限），恭喜你找到根因了：连接数打满了！
# RTD 连接机台，每个 EAP 通讯就是一个 socket，很容易触达上限

# 3. 看它的启动命令
cat /proc/12345/cmdline | tr '\0' ' '
# java -Xmx2g -jar rtd-engine.jar --spring.profiles.active=prod
# 哦，Xmx 才 2G？难怪 OOM！
```
...
```bash
cat /proc/loadavg    # 1min 5min 15min 负载，最后是 运行线程/总线程 和最新PID
cat /proc/meminfo    # 内存全景：可用、缓存、交换区，比 free 命令还详细
cat /proc/cpuinfo | grep "model name" | head -1  # 什么 CPU，有没有超线程
```

④ 实战：定位「文件描述符泄漏」

RTD 服务的经典故障：跑着跑着连接不上机台了。进容器执行：

```bash
PID=$(pgrep -f rtd-engine)
ls /proc/$PID/fd/ | wc -l              # 发现快满了
ls -l /proc/$PID/fd/ | grep socket | wc -l  # 全是 socket！
# → 根因：某个机台断连后代码没 close socket，fd 只增不减
```

没有 /proc，这些信息你得上 strace 或者 jstack 慢慢抠。/proc 就是那个「站在巨人肩膀上」的巨人。

```mermaid
graph LR
    A["/proc/pid/status<br/>进程体检"] --> B["/proc/pid/fd/<br/>文件描述符"]
    B --> C["/proc/meminfo<br/>系统内存"]
    A --> D["/proc/loadavg<br/>系统负载"]
    style B fill:#f96,stroke:#333
```

⑤ 一个重要限制

/proc 是实时快照，不是历史记录。进程一死，/proc/PID/ 目录立刻消失。所以排查 OOM 被 kill 掉的进程，事后是看不到 /proc 数据的——这就是为什么需要监控系统（Prometheus、Telegraf）持续采集。

💡 记住一句话：/proc 是 Linux 给你的内核级调试器——零依赖、零开销，随时可用。进程的内存、线程、文件描述符，全在 /proc/PID/ 里摊开了给你看。

❓ 今日一题
Q: RTD 服务 PID 12345，想快速知道它打开了多少个文件描述符（含 socket 连接），哪个命令最快？
A) lsof -p 12345 | wc -l
B) ls /proc/12345/fd/ | wc -l
C) cat /proc/12345/limits
D) cat /proc/12345/status | grep FD

---

## 17:03 | 性能

### sar系统性能监控：RTD排障的行车记录仪

上期答案: B — `ls /proc/12345/fd/ | wc -l` 直接统计文件描述符数量，零开销。A 的 `lsof` 不仅要枚举 fd 还要解析 socket 协议、inode 等信息，慢一个数量级；C 的 `/proc/pid/limits` 显示的是软硬上限值（比如 1024），不是当前使用量；D 的 `/proc/pid/status` 根本没有 FD 相关字段。RTD 现场排查，B 就是你最快的手指。\n\n---\n\n📚 [性能] sar — RTD 系统的「行车记录仪」\n\n你有没有遇到过这种场景：RTD 派工服务半夜卡了 5 分钟，等你被告警叫醒的时候它自己又好了。top 看一切正常，日志也没报错——就像车被蹭了但行车记录仪没开，只能干瞪眼。\n\n这就是 sar（System Activity Reporter）的价值。它像一个不眠不休的记录仪，每隔一段时间自动采集 CPU、内存、IO、网络的数据存起来，出事后你能「倒带」回看案发时刻的系统状态。\n\n**装车 — 启用 sar 数据采集**\n\n```bash\nyum install sysstat -y          # 装上记录仪\nsystemctl enable --now sysstat  # 点火，开始记录\n```\n\n默认每 10 分钟采集一次，存到 `/var/log/sa/` 目录。sa 开头是二进制数据（给 sar 读的），sar 开头是人类可读的文本报告。\n\n**回放 — 看历史数据**\n\n```bash\nsar -f /var/log/sa/sa28   # 看今天全天数据\nsar -u -s 02:00:00 -e 03:30:00 -f /var/log/sa/sa28  # 只看凌晨2点到3点半\n```\n\n参数 `-u` 看 CPU 利用率：\n\n```\n02:10:01   %user  %system  %iowait   %idle\n02:20:01    12.3     5.1      0.2    82.4\n02:30:01    15.7     8.2      0.5    75.6\n02:40:01    98.4    35.2     45.3     1.1   ← 这里！\n```\n\n凌晨 2:40 突然 `%iowait` 冲到 45%，`%user` 到 98%——正好对应告警时间。再换个维度看：\n\n```bash\nsar -r -f /var/log/sa/sa28  # 看内存：-r\nsar -b -f /var/log/sa/sa28  # 看磁盘IO：-b\nsar -n DEV -f /var/log/sa/sa28  # 看网络流量：-n DEV\n```\n\n**RTD 实战案例**\n\nTrace 数据落盘时 RTD 服务间歇性卡顿。运维看了半天 top 没发现问题——因为 top 是实时快照，看不出来。\n\n```bash\nsar -b -f /var/log/sa/sa28 | awk '$5>1000 {print $1,$2,$5,$6}'\n# 12:40:01 tps=3500 rtps=1200 wtps=2300 → 磁盘写 IOPS 爆炸\n```\n\n真相大白：某批 Trace 文件集中落盘时，磁盘 IOPS 撑不住了。解决方案：加 disk IO 限流或换 SSD。sar 帮你从「盲人摸象」变成「回看监控录像」。\n\n```mermaid\ngraph LR\n    A[\"sysstat 守护进程<br/>每10分钟采集\"] --> B[\"/var/log/sa/saDD<br/>二进制数据\"]\n    B --> C[\"sar -u -r -b -n<br/>多维分析\"]\n    C --> D[\"定位性能根因\"]\n    style A fill:#39f,stroke:#333,color:#fff\n    style C fill:#f96,stroke:#333\n```\n\n**一个限制**：sar 默认粒度是 10 分钟，如果你的故障持续只有 30 秒，可能正好落在两次采集之间。这时需要配合 `pidstat`（同为 sysstat 套件，默认 1 秒粒度）做进程级监控：\n\n```bash\npidstat -p $(pgrep -f rtd-engine) 1  # 每秒采集一次\n```\n\nsar 管全局趋势，pidstat 管进程细节——一远一近，互补配合。\n\n💡 记住一句话：top 看到的是现在，sar 看到的是历史——没有历史数据支撑的故障排查，等于闭着眼睛开车。\n\n---\n\n❓ 今日一题\nQ: sar 默认每多少分钟采集一次系统性能数据？\nA) 1 分钟\nB) 5 分钟\nC) 10 分钟\nD) 30 分钟
